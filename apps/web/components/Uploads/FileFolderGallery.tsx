...
function FileFolderGallery({ options: { folder = "", entityID = "", collection = "" }, onCompleteCallback = (f: any) => { } }: any) {
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-2">
                        {folders.map((file: any) => {
                            ...
                            return file.Key !== folderPath + "/" && <div
                                key={file.Key} className="relative group border rounded p-2 hover:bg-gray-50"
                            >
                                <div className="flex items-center justify-between">
                                    <p
                                        className={"text-center font-semibold cursor-pointer flex-1"}
                                        onClick={(e: any) => {
                                            e.preventDefault()
                        ...
                        {files.map((file: any) => (
                            <div key={file.Key} className="relative group">
                                ...
                                <div className={"cursor-pointer absolute inset-0 flex items-center justify-center " + (isPDF(file) ? "hidden" : "bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity duration-300")}>
                                    {!isPDF(file) && <a href={file.src} target="_blank" className="text-center cursor-pointer" style={{ width: "90%" }}>
...
export default memo(FileFolderGallery)